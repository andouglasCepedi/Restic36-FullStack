...
class Column {
    ...
    moveTask(taskId: number, targetColumn: Column): void {
    ...
    render(): void {
        const listElement = document.getElementById(`${this.id}-list`) as HTMLUListElement; //Pega o elemento ul da coluna na página HTML
        listElement.innerHTML = ''; //Apaga todos os itens
        this.tasks.forEach(task => { //Passa por todas as tasks no array tasks
            ...
            listItem.onclick = () => { //Cria o listener para quando o botão for clicado
...
function addTask(columnId: string): void {
    ...
    if (title) {
        ...
        if (column) {
            ...
            const task = new Task(taskId, title); //Cria uma nova task
            column.addTask(task); //Adiciona a task criada a coluna
        }
    }
}
